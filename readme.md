...
USB メモリを挿したまま起動するとセットアップスクリプトが再実行され、USB 内の `mqtt_bridge.py` と `config.json` が本体の `/data/local/` に再コピーされます。ADB で本体のブリッジを更新した場合は、USB 内の同じファイルも更新してください。
...
| `force_publish_interval` | 値が変化しない場合もMQTTへ再送する最大間隔（秒、既定60秒） |
| `target_pan_id` | （任意）確認済みメーターの PAN ID。他の PAN には接続を試みません |
| `target_channel` | （任意）実測したチャンネル番号。以下の3項目がすべてある場合は30分に1回まで直接接続を試し、失敗時は通常走査に戻ります。再走査時はチャンネル変更を許容します |
| `target_addr` | （任意）確認済みメーターの MAC アドレス（16進文字列）。他のアドレスには接続を試みません |

メーターが見つからない場合は、Wi-SUN 走査の間隔を1分から最大15分まで段階的に延ばします。
...
9. **本体内監視**: 30秒ごとのブリッジ生存確認、Wi-Fi再設定、Wi-SUN応答監視、回数制限付きのブリッジ・本体再起動を行う
...
├── cube_j1_watchdog.sh      # Cube本体だけで動く低負荷の自己復旧監視
├── cube_j1_watchdog.rc      # 自己復旧監視をブート時に起動するinit設定
...
- **Wi-SUN 接続**: スキャン結果の PairID が B ルート認証 ID の末尾8文字と一致するメーターだけに接続を試みます。`target_pan_id` と `target_addr` を設定した場合は両方の一致も必須です。集合住宅で近隣のメーターが見えても接続試行しません。チャンネルは変わる場合があるため、一致条件にしません。
- **Wi-SUN 自動復旧**: スマートメーターからの応答が6回連続でタイムアウトした場合、Wi-SUN を初期化して PAN を再スキャン・再接続します。単発のタイムアウトでは再接続しません。
- **長期障害時の負荷抑制**: メーターが見つからない場合の再試行間隔は 1、2、4、8、最大15分へ延ばし、無線モジュールの連続スキャンとリセットを抑えます。
- **状態連携**: RAM上の `/tmp/mqtt_bridge.health` と MQTT の `cubej/{device_id}/status` に、ブリッジ、MQTT、メーター、Wi-SUN UARTの最終応答時刻を記録します。Debian監視はこれを使いますが、状態通知がない場合もADB/SSHと電力MQTTだけで復旧判定を続けます。30秒ごとの生存確認では本体フラッシュを書き換えません。
- **メモリ制限**: 生存確認とMQTT状態送信は1本の補助スレッドで行い、補助スレッドのスタックを128KBに制限します。MQTT切断中の送信待ちはトピックごとに最新1件だけ保持するため、長期障害でも増え続けません。
- **MQTT送信**: いずれかの測定値が変わればすぐ送信し、変化がなくても既定で60秒に1回は再送します。Home Assistantの表示とDebian監視を継続しつつ、同一値の過剰送信を抑えます。
- **本体内監視**: ブリッジの生存ファイルが4分止まるとブリッジを再起動します。Wi-SUN UARTが30分応答しない場合もブリッジを再起動し、3回続いた場合だけ本体を再起動します。本体再起動は6時間に1回までです。MQTT切断中にWi-FiアドレスまたはLAN経路が消えた場合は10分間隔で再設定し、1時間戻らない場合だけ本体を再起動します。Debianだけが停止している場合はゲートウェイ疎通が残るためCubeを再起動しません。
- **動作ログ**: ブリッジログは `/data/local/mqtt_bridge.log`、本体内監視ログは `/data/local/cube_j1_watchdog.log` に残ります。測定値の記録は5分間隔とし、サイズ上限でローテーションします。B ルート認証情報と近隣メーターの識別子はログで伏せます。
- **温度・資源ログ**: Cube J1の固定RK3128カーネルはTSADCを公開しておらず、BP35C0のUART仕様にも温度取得コマンドがないため、どちらの温度も取得できません。この事実を監視起動時に1回記録し、15分ごとにCPU周波数、1分負荷、空きRAMを本体内監視ログへ記録します。Wi-SUNモジュールは最終応答時刻とスキャン時LQIを監視します。
...
| Cube状態・各最終応答時刻 | `cubej/{device_id}/status` |
| MQTT接続状態 | `cubej/{device_id}/availability` |
